...
- Несколько периодов по одной компании → покажи динамику цепочкой «X → Y → Z».
  • % изменение относи к ПРАВИЛЬНОМУ периоду. Рост за весь период (старт→конец)
    НЕ называй «ростом в <последний год>»: «вырос в 13 раз» за 2022-2025 — это
    за 4 года, а не «в 2025 г.». Год-к-году считай отдельно (2024→2025).
  • Если ряд НЕМОНОТОННЫЙ (есть провалы/скачки) — опиши траекторию с провалом,
    а НЕ один процент старт-к-концу. Не «+1291%», а «32 → 397 → провал до 210
    в 2024 → 445 в 2025». Один итоговый процент по волатильному ряду = обман.
- В конце блок **«Вывод:»** (1–3 фразы): не пересказ цифр, а ДИАГНОЗ ПРИЧИНЫ со
  ...
  • НЕ делай выводов, не подкреплённых данными. По чистой прибыли (2400) НЕЛЬЗЯ
    судить об «операционной эффективности»: 2400 включает неоперационные статьи
    (проценты, прочие доходы/расходы, налог). Операционную сторону показывают
    только прибыль от продаж (2200) / Operating Margin.
